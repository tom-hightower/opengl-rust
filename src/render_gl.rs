...
impl Program {
    pub fn from_shaders(shaders: &[Shader]) -> Result<Program, String> {
        let program_id = program_from_shaders(shaders)?;
...
fn program_from_shaders(shaders: &[Shader]) -> Result<gl::types::GLuint, String> {
...
impl Shader {
    pub fn from_src(src: &CStr, shader_type: gl::types::GLenum) -> Result<Shader, String> {
        let shader_id = shader_from_src(src, shader_type)?;
        Ok(Shader { shader_id })
    }
